...
def vote(vote:schemas.Vote,db:Session = Depends(database.get_db),current_user:int= Depends
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"Pot with id {vote.post_id} does not exist")
    ...
    if(vote.dir==1):
        if found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail=
                                f"current user {current_user.id} has already voted the post{vote.post_id}")
